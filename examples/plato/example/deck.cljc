(ns plato.example.deck
  "The deck plato uses to explain itself.
   ...
   A .cljc so the SAME value the Reagent shell mounts is the one the build step
   prerenders on the JVM or the native binary — plato.example.core is only the
   browser entry point that mounts it."
...
(def model
  (deck/deck
   {:title "Plato — a deck is a value"
    :description "Plato — ClojureScript presentations with live Desargues animation"
    :config {:hash true
             :history true
             :controls true
             :progress true
